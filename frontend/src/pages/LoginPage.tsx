...
interface Props {
  onSuccess: (token: string, email: string, role: string, tenantName: string) => void;
}
...
export default function LoginPage({ onSuccess }: Props) {
  ...
  const [tenantSlug, setTenantSlug] = useState(slugFromState);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess(res.token, res.email, res.role, res.tenantName);
  ...
  return (
    <AuthLayout title={t('auth.loginTitle')} subtitle={t('auth.loginSubtitle')}>
      ...
      <a
        href={googleLoginUrl()}
        className="flex w-full items-center justify-center gap-3 rounded-xl border border-slate-300 py-2.5 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-slate-50 dark:border-white/10 dark:text-slate-200 dark:hover:bg-white/5"
      >
